...
import SubstituteAssignment from '../substitute/SubstituteAssignment';
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          ...
          <div className="space-y-2 text-sm">
            {awaitingCerts.map(absence => (
              <div key={absence.id} className="bg-white p-3 rounded-lg flex justify-between items-center border border-yellow-200">
                <div>
                ...
                <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded">ממתין לאישור</span>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={absence.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    className="text-yellow-700 border-yellow-300 hover:bg-yellow-50 flex-1"
                  >
                    <XCircle className="h-4 w-4 mr-1" />
                    דחה
                  </Button>
                </div>
                {isEnabled('substitutes') && (
                  <SubstituteAssignment absence={absence} onAssigned={() => queryClient.invalidateQueries({ queryKey: ['absences'] })} />
                )}
              </div>
            ))
          ) : (
