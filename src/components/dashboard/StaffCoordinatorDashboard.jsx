...
export default function StaffCoordinatorDashboard({ user, setView }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in relative">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-br from-yellow-600 via-blue-600 to-blue-600 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-yellow-100 text-sm opacity-90">עובדת הוראה + רכזת - {user.title || ''}</p>
            </div>
          </div>
      ...
      {/* Daily Schedule */}
      <div className="bg-gradient-to-br from-white to-yellow-50/30 rounded-2xl shadow-lg border border-yellow-100 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="text-xs font-bold text-yellow-700 hover:text-yellow-700 bg-yellow-50 hover:bg-yellow-100 px-4 py-2 rounded-lg transition-all border border-yellow-200 shadow-sm"
          >
            📅 יומן חודשי מלא
          </button>
        </div>
            ...
            return (
              <div key={hour} className="min-w-[130px] flex-1">
                ...
                <div className={`p-4 rounded-b-xl border-2 text-center h-24 flex flex-col justify-center items-center gap-1.5 transition-all ${
                  lesson && !isEmpty
                    ? 'bg-gradient-to-br from-yellow-50 to-blue-50 border-yellow-200 shadow-md hover:shadow-lg hover:scale-105' 
                    : lesson 
                  ...
                  {lesson ? (
                    <>
                      ...
                      {lesson.includes('-') && (
                        <span className="text-xs text-yellow-700 bg-white px-2.5 py-0.5 rounded-full font-semibold border border-yellow-100">
                          {lesson.split('-')[1]?.trim()}
                        </span>
                      )}
                    </>
      ...
      {myDuty && (
        <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-yellow-900">תורנות היום</h4>
              <p className="text-sm text-yellow-800">{myDuty.duty_type} בשעה {myDuty.time}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-lg border border-yellow-100 p-6">
          ...
          <div className="space-y-3">
            <button className="w-full flex items-center justify-between p-4 bg-yellow-50 rounded-xl hover:bg-yellow-100 border-2 border-yellow-200 hover:border-yellow-300 transition-all group">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white rounded-full text-yellow-700 shadow-sm">
              ...
              <ArrowRight className="h-4 w-4 text-yellow-600 group-hover:text-yellow-700" />
            </button>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-yellow-100 p-6">
          <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
            <div className="p-2 bg-yellow-100 rounded-lg">
              <FileSignature className="h-5 w-5 text-yellow-700" />
            </div>
            העלאת קבצים לחתימה
          </h3>
          <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 transition-colors cursor-pointer group">
            <Upload className="h-8 w-8 text-slate-300 mx-auto mb-2 group-hover:text-yellow-600" />
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
          <div className="h-8 w-8 bg-gradient-to-br from-yellow-500 to-blue-500 rounded-lg flex items-center justify-center">
            <span className="text-white text-sm">⚡</span>
          </div>
          פעולות מהירות
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-9 gap-3">
          <button onClick={() => setAbsenceModalOpen(true)} className="relative overflow-hidden flex flex-col items-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-50 rounded-xl shadow-sm border-2 border-yellow-200 hover:border-yellow-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="absolute top-1 right-1 bg-yellow-500 text-[9px] text-white font-bold px-2 py-0.5 rounded-full shadow">חשוב</div>
          ...
          <button onClick={() => openFeature('substitute')} className="flex flex-col items-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-50 rounded-xl shadow-sm border-2 border-yellow-200 hover:border-yellow-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="p-2.5 bg-yellow-100 rounded-full text-yellow-700 group-hover:scale-110 transition-transform mb-2">
          ...
          <button onClick={() => openFeature('external')} className="flex flex-col items-center p-4 bg-gradient-to-br from-green-50 to-green-50 rounded-xl shadow-sm border-2 border-green-200 hover:border-green-400 hover:shadow-md transition-all group text-center h-28 justify-center">
          ...
          <button onClick={() => setPrintModalOpen(true)} className="flex flex-col items-center p-4 bg-gradient-to-br from-blue-50 to-blue-50 rounded-xl shadow-sm border-2 border-blue-200 hover:border-blue-400 hover:shadow-md transition-all group text-center h-28 justify-center">
          ...
          <button onClick={() => openFeature('purchase')} className="flex flex-col items-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-50 rounded-xl shadow-sm border-2 border-yellow-200 hover:border-yellow-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="p-2.5 bg-yellow-100 rounded-full text-yellow-700 group-hover:scale-110 transition-transform mb-2">
          ...
          <button onClick={() => openFeature('maintenance_pc')} className="flex flex-col items-center p-4 bg-gradient-to-br from-blue-50 to-green-50 rounded-xl shadow-sm border-2 border-blue-200 hover:border-blue-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="p-2.5 bg-blue-100 rounded-full text-blue-600 group-hover:scale-110 transition-transform mb-2">
          ...
          <button onClick={() => openFeature('overtime')} className="flex flex-col items-center p-4 bg-gradient-to-br from-yellow-50 to-yellow-50 rounded-xl shadow-sm border-2 border-yellow-200 hover:border-yellow-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="p-2.5 bg-yellow-100 rounded-full text-yellow-700 group-hover:scale-110 transition-transform mb-2">
          ...
          <button onClick={() => openFeature('special_overtime')} className="flex flex-col items-center p-4 bg-gradient-to-br from-blue-50 to-yellow-50 rounded-xl shadow-sm border-2 border-blue-200 hover:border-blue-400 hover:shadow-md transition-all group text-center h-28 justify-center">
            <div className="p-2.5 bg-blue-100 rounded-full text-blue-600 group-hover:scale-110 transition-transform mb-2">
