...
export function PrintUploadForm({ user, onClose }) {
  const queryClient = useQueryClient();
  const [uploading, setUploading] = useState(false);
  const [formData, setFormData] = useState({
    file: null, file_name: '', subject: '', class_name: '',
    copies: 1, pages_per_copy: 1, urgent: false
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.file) { toast.error('נא לבחור קובץ'); return; }
    setUploading(true);
    try {
      ...
      await base44.entities.PrintRequest.create({
        ...
        file_url, file_name: formData.file_name,
        subject: formData.subject, class_name: formData.class_name,
        copies: formData.copies, pages_per_copy: formData.pages_per_copy,
        total_pages, urgent: formData.urgent, status: 'pending'
      });
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
                placeholder="לדוגמה: מתמטיקה" required />
            </div>
            <div>
              ...
              <Input value={formData.class_name} onChange={(e) => setFormData({ ...formData, class_name: e.target.value })}
                placeholder="לדוגמה: ח'2" required />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-2">כמות עותקים</label>
              <Input type="number" min="1" value={formData.copies}
                onChange={(e) => setFormData({ ...formData, copies: parseInt(e.target.value) })} required />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-2">עמודים בקובץ</label>
              <Input type="number" min="1" value={formData.pages_per_copy}
                onChange={(e) => setFormData({ ...formData, pages_per_copy: parseInt(e.target.value) })} required />
            </div>
          </div>
          <div className="bg-slate-50 p-4 rounded-lg">
            <p className="text-sm text-slate-600">
              סה״כ דפים להדפסה: <span className="font-bold text-blue-600">{formData.copies * formData.pages_per_copy}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            <input type="checkbox" id="urgent" checked={formData.urgent}
              onChange={(e) => setFormData({ ...formData, urgent: e.target.checked })} className="w-4 h-4" />
            <label htmlFor="urgent" className="text-sm font-medium text-slate-700">דחוף - נדרש בהקדם</label>
          </div>
